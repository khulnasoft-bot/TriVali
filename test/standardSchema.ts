...
function verifyStandardSchema<Input, Output>(
  ...
  return (
    ...
    schema['~standard'].vendor === 'trivali' &&
    typeof schema['~standard'].validate === 'function'
  );
}

/**
 * Helper function to get Trivali validation error directly without try-catch boilerplate
 */
async function getTrivaliValidationError(
  schema: any,
  value: any,
  options?: any,
): Promise<any> {
...
/**
 * Helper function to compare standard schema validation results with main Trivali API
 */
async function expectValidationConsistency(
  ...
  // Test with main API
  const mainAPIError = await getTrivaliValidationError(schema, testValue, options);
