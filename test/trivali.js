...
describe('Trivali', function () {
  ...
  it('should REACH correctly', async () => {
    ...
    expect(() => reach(inst, 'nested.arr[1].num')).toThrowError(
      'Trivali.reach cannot implicitly index into a tuple type. the path part ".nested" must contain an index to the tuple element, e.g. ".nested[0]"',
    );
