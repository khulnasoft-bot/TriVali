...
class ValidationError extends Error {
  ...
  static [(_c = Symbol.hasInstance, _b = Symbol.toStringTag, _c)](inst) {
...
const isSchema = (obj) => obj && obj.__isTrivaliSchema__;
class Condition {
  ...
  static fromOptions(refs, config) {
    ...
    return new Condition(refs, (values, schema) => {
      ...
      return (branch == null ? void 0 : branch(schema)) ?? schema;
    });
  }
...
class Reference {
  ...
  static isRef(value) {
    return value && value.__isTrivaliRef;
  }
}
Reference.prototype.__isTrivaliRef = true;
...
function getIn(schema, path, value, context = value) {
  ...
  expr.forEach(path, (_part, isBracket, isArray) => {
    ...
    if (schema.innerType || isTuple) {
      if (isTuple && !isArray)
        throw new Error(
          `Trivali.reach cannot implicitly index into a tuple type. the path part "${lastPartDebug}" must contain an index to the tuple element, e.g. "${lastPartDebug}[0]"`
        );
      if (value && idx >= value.length) {
        throw new Error(
          `Trivali.reach cannot resolve an array item at index: ${_part}, in the path: ${path}. because there is no value at that index. `
        );
      }
      parent = value;
...
function issuesFromValidationError(error, parentPath) {
  var _a2;
  if (!((_a2 = error.inner) == null ? void 0 : _a2.length) && error.errors.length) {
...
` + (formattedResult !== formattedValue ? `result of cast: ${formattedResult}` : "")
  ...
  _cast(rawValue, options) {
    let value = rawValue === void 0 ? rawValue : this.transforms.reduce(
  ...
  validate(value, options) {
    ...
    let disableStackTrace = (options == null ? void 0 : options.disableStackTrace) ?? schema.spec.disableStackTrace;
    return new Promise(
      (resolve, reject) => schema._validate(
        value,
        options,
        (error, parsed) => {
  ...
  required(message = mixed.required) {
    return this.clone().withMutation(
      (next) => next.nonNullable(message).defined(message)
    );
  }
  notRequired() {
  ...
  when(keys, options) {
    ...
    next.conditions.push(
      typeof options === "function" ? new Condition(deps, options) : Condition.fromOptions(deps, options)
    );
    return next;
  }
  typeError(message) {
  ...
  describe(options) {
    ...
    const description = {
      ...
      tests: next.tests.filter(
        (n, idx, list) => list.findIndex((c) => c.OPTIONS.name === n.OPTIONS.name) === idx
      ).map((fn) => {
        const params = fn.OPTIONS.params && options ? resolveParams({ ...fn.OPTIONS.params }, options) : fn.OPTIONS.params;
        return {
...
Schema.prototype.__isTrivaliSchema__ = true;
...
class MixedSchema extends Schema {
  constructor(spec) {
    super(
      typeof spec === "function" ? { type: "mixed", check: spec } : { type: "mixed", check: returnsTrue, ...spec }
    );
  }
}
...
function parseDateStruct(date2) {
  ...
  return {
    ...
    precision: ((_a2 = regexResult[7]) == null ? void 0 : _a2.length) ?? void 0,
...
class NumberSchema extends Schema {
  ...
  round(method) {
    ...
    method = (method == null ? void 0 : method.toLowerCase()) || "round";
...
const _DateSchema = class _DateSchema extends Schema {
  constructor() {
    super({
      type: "date",
      check(v) {
...
_DateSchema.INVALID_DATE = invalidDate;
let DateSchema = _DateSchema;
...
function findIndex(arr, err) {
  ...
  arr.some((key, ii) => {
    var _a2;
    if ((_a2 = err.path) == null ? void 0 : _a2.includes(key)) {
      idx = ii;
      return true;
    }
  });
...
const deepHas = (obj, p) => {
  const path = [...expr.normalizePath(p)];
  if (path.length === 1) return path[0] in obj;
  let last = path.pop();
  let parent = expr.getter(expr.join(path), true)(obj);
...
class ObjectSchema extends Schema {
  ...
  _getDefault(options) {
    ...
    this._nodes.forEach((key) => {
      ...
      if (innerOptions == null ? void 0 : innerOptions.value) {
        innerOptions = {
          ...innerOptions,
  ...
  from(from, to, alias) {
    let fromGetter = expr.getter(from, true);
...
class ArraySchema extends Schema {
  ...
  of(schema) {
    ...
    if (!isSchema(schema))
      throw new TypeError(
        "`array.of()` sub-schema must be a valid trivali schema not: " + printValue(schema)
      );
...
class Lazy {
  constructor(builder) {
    ...
    this.spec = { meta: void 0, optional: false };
  }
  clone(spec) {
  ...
  describe(options) {
    return options ? this.resolve(options).describe(options) : { type: "lazy", meta: this.spec.meta, label: void 0 };
  }
  meta(...args) {
    if (args.length === 0) return this.spec.meta;
    let next = this.clone();
...
function addMethod(schemaType, name, fn) {
  if (!schemaType || !isSchema(schemaType.prototype))
    throw new TypeError("You must provide a trivali schema constructor function");
